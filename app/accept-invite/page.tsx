...
function AcceptInviteForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!token) {
      setError("Missing invite token. Open the link from your email or contact your administrator.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok || !data.ok) {
        setError(
          data.message ??
            "This invite link is invalid, expired, or already used. Ask your administrator for a new invite.",
        );
        setPending(false);
        return;
      }
  ...
  if (!tokenFromQuery) {
    return (
      <p className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-100">
        Missing invite token. If you were sent an email, open the link from that message. Need a password reset for an
        existing account? Try{" "}
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div>
        <label htmlFor="invite-password" className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">
          Choose a password
        </label>
        <input
          id="invite-password"
      ...
      {error ? (
        <p id="invite-error" className="text-sm text-red-600 dark:text-red-400" role="alert">
          {error}
        </p>
      ) : null}
      <button
...
export default function AcceptInvitePage() {
  return (
    <div className="mx-auto w-full max-w-md space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-zinc-900 dark:text-zinc-50">Accept your invite</h1>
        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Set a password to finish setting up your account.</p>
      </div>
